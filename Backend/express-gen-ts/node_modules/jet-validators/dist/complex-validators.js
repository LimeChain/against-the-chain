...
const isEnumVal = (arg) => _isEnumVal(arg, false, false, false);
exports.isEnumVal = isEnumVal;
const isOptionalEnumVal = (arg) => _isEnumVal(arg, true, false, false);
exports.isOptionalEnumVal = isOptionalEnumVal;
const isNullableEnumVal = (arg) => _isEnumVal(arg, false, true, false);
exports.isNullableEnumVal = isNullableEnumVal;
const isNullishEnumVal = (arg) => _isEnumVal(arg, true, true, false);
exports.isNullishEnumVal = isNullishEnumVal;
const isEnumValArray = (arg) => _isEnumVal(arg, false, false, true);
exports.isEnumValArray = isEnumValArray;
const isOptionalEnumValArray = (arg) => _isEnumVal(arg, true, false, true);
exports.isOptionalEnumValArray = isOptionalEnumValArray;
const isNullableEnumValArray = (arg) => _isEnumVal(arg, false, true, true);
exports.isNullableEnumValArray = isNullableEnumValArray;
const isNullishEnumValArray = (arg) => _isEnumVal(arg, true, true, true);
exports.isNullishEnumValArray = isNullishEnumValArray;
function _isEnumVal(enumArg, optional, nullable, isArray) {
    if (!(0, exports.isEnum)(enumArg)) {
        throw Error('Item to check from must be an enum.');
    }
    let enumVals = Object.keys(enumArg).reduce((arr, key) => {
        if (!arr.includes(key)) {
            arr.push(enumArg[key]);
        }
        return arr;
    }, []);
    if ((0, basic_1.isNumber)(enumArg[enumVals[0]])) {
        enumVals = enumVals.map(item => enumArg[item]);
    }
    const test = (arg) => enumVals.some(val => arg === val);
    return (arg) => {
        if ((0, basic_1.isUndef)(arg)) {
            return !!optional;
        }
