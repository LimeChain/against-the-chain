...
func handle_direction(direction: Vector2):
	var sprite_element: Sprite2D = $EnemyBody/Sprite2D
	
	if direction.x < 0:
	#and ((direction.y > 0 and direction.y < 0.5) or ((direction.y < 0 and direction.y > -0.5))):
		sprite_element.texture = preload("res://assets/mobs/Mob2-W-10.svg")
	elif direction.x > 0:
	#and ((direction.y > 0 and direction.y < 0.5) or ((direction.y < 0 and direction.y > -0.5))):
		sprite_element.texture = preload("res://assets/mobs/Mob2-E-10.svg")
